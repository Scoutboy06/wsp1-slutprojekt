...
class Media < Collection
  ...
  def update(id:, data:, id_expr: '?')
    ...
    old_media_data = execute_sql("SELECT file_path FROM media WHERE id = (#{id_expr})", [id], debug: true).first
  ...
  def delete(id_expr: '?', id:)
    ...
    execute_sql("DELETE FROM media WHERE id = (#{id_expr})", values)
  end

  def delete_by_id_expr(id_expr:, values:)
    media_data = execute_sql("SELECT file_path FROM media WHERE id = (#{id_expr})", values).first
    return if media_data.nil?
