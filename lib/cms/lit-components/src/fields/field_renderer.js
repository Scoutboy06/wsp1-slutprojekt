...
export default class FieldRenderer extends LitElement {
	...
	static renderField(field, mode) {
		switch (field.type) {
			...
			case "relation":
				return html`<relation-field .name=${field.name} .fields=${field.relation_field} .defaultValue=${field.default} .required=${field.required} .mode=${mode}></relation-field>`;
			default:
				return null;
		}
	}
	...
	render() {
      ...
      <div class="flex flex-col gap-4">
        ${fields.map((field) => {
					return html`${FieldRenderer.renderField(field, mode)}`;
				})}
      </div>
    `;
	}
